...
__weak void Can_WifiRx_Save(uint8_t res)
{
	if(Wifi_Rx_flag == 0)
	{
...
__weak void Normal_data(void)	  // 正常接收8字节控制指令
{
	uint8_t sum=0;
...
__weak void Abnormal_data(void)	  //数据异常处理
{
	uint8_t i,j;
	uint8_t sum=0;
...
//	if(Wifi_Rx_flag)
//	{	
//		if(gt_get_sub(canu_wifi_rxtime) == 0)    //每10ms执行一次
//		{
