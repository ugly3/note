...
Communication_Typedef Communication_Data =
    {
        ...
        .FollowCar_Start_flag = 0,              // 从车启动标志位
};
    ...
    if (Zigbee_Rx_flag)
    {
        if (gt_get_sub(canu_zibe_rxtime) == 0)
        {
            ...
            if (0x02 == Zigb_Rx_Buf[1]) //*****************接收主车发过来的数据
            {
                ...
                {
                    if (0x01 == Zigb_Rx_Buf[3]) // 从车启动
                    {
                        Communication_Data.FollowCar_Start_flag = 1;
                        MainCar_Data.xSend_Command_To_MainCar(FllowCar_start_Finish); // 发送从车完成启动
                    }
                    else if (0x02 == Zigb_Rx_Buf[3]) // 主车完成启动
                        MainCar_Data.MainCar_Start_Finish_Flag = 1;
                }
                else if (0xA1 == Zigb_Rx_Buf[2] && 0xA2 == Zigb_Rx_Buf[3]) // 接收主车发送的温度数据
                {
                }
                else if (0xA2 == Zigb_Rx_Buf[2] && 0xA3 == Zigb_Rx_Buf[3]) // 接收主车发送的距离数据
                {
                }
                ...
                {
                    MainCar_Send_Init_light = Zigb_Rx_Buf[3]; // 主车传来的路灯初始档位信息
                }
                else if (0x02 == Zigb_Rx_Buf[2]) // 主车传来的车库层数信息
                {
