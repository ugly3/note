...
uint8_t CT[2];		// 卡类型
uint8_t SN[4];		// 卡号
char READ_RFID[16];		// 存放RFID读到的数据
char Judge_READ_RFID[16];	// 存放RFID读到的数据,用来判断卡片的真假 

char READ_RFID1[16];     //存放第一张卡读到的数据(缓存)
char READ_RFID2[16];     //存放第二张卡读到的数据(缓存)
char READ_RFID3[16];     //存放第三张卡读到的数据(缓存)
char Judge_READ_RFID1[7];     //存放第一张卡读到的数据，用来判断卡片的真假 
char Judge_READ_RFID2[7];     //存放第二张卡读到的数据 ，用来判断卡片的真假 


uint8_t WRITE_RFID[16]={"**6&&2^^<D3>"};        //存放RFID写入的数据
...
{
	uint8_t card_key = (card_addr/4)*4+3;   //计算RFID卡片中密钥块（KEY块）地址
...
//					Send_InfoData_To_Fifo((char *)ADDR_Str,14);		// 印读写操作块地址
					...
					if((mode == RFID_Read) || (mode == RFID_Write_Read))	// 允许读出数据
					{
						...
						{
#if 1
                            if(PcdRead(10,Judge_READ_RFID) == MI_OK)
                            {
...
char InitRc522(void) //RC522读卡器初始化（主要是对RC522读卡芯片的设置），在此函数中存在于RC522读卡器的数据交互
{
	if(PcdReset() != MI_OK)
		return MI_ERR;
	PcdAntennaOff();
...
//返    回: 成功返回MI_OK
///////////////////////////////////////////////////////////////////// 
char PcdRead(unsigned char addr,char *pData)
{
    char status;
    unsigned int  unLen;
    unsigned char i,ucComMF522Buf[MAXRLEN]; 
