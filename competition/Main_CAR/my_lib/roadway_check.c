...
volatile uint8_t Stop_Flag = 0;                  //停止标志位
...
uint32_t temp_MP = 0;                  //码盘值临时变量
uint32_t temp_Nav = 0;                 //码盘旋转值临时变量

volatile uint32_t dis_MP = 0;
...
volatile uint16_t Count_ms = 0;//遇到特殊标志物或十字路口时的时间，根据时间长短，从而判断出特殊标志物在哪个坐标点
//_________________________________________________________
volatile int32_t Roadway_cmp;                    //最终码盘值
extern volatile int32_t CanHost_Mp;             //码盘临时变量值
...
volatile uint16_t track_time_ms=0;
volatile uint8_t track_time_Start=0;
