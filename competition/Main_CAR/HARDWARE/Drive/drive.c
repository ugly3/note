...
*****************************************************************/
uint8_t Light_Inf(uint8_t gear)        //(冒泡排序)
{
	uint8_t i;
...
void Car_Left(uint8_t speed)       // 主车左转 参数：速度       //左右转用循迹灯来判断，不用码盘
{
	delay_ms(100);
    Stop_Flag = 0;          // 运行状态标志位
    Go_Flag = 0;            // 前进标志位
...
void Car_L45(int8_t speed, uint16_t times)		// 主车左转 参数：速度，旋转时间     80    450
{
    Car_Spend = speed;              // 速度值
    Control(-Car_Spend, Car_Spend);  // 电机驱动函数 
    delay_ms(times);
...
void Car_R45(int8_t speed, uint16_t times)		// 主车右转 参数：速度，旋转时间     80    450
{
    delay_ms(100);
    Car_Spend = speed;              // 速度值
    Control(Car_Spend, -Car_Spend);  // 电机驱动函数 
...
void Car_Track(int8_t speed)         //主车循迹函数            
{
    Stop_Flag = 0;          // 运行状态标志位
    Go_Flag = 0;            // 前进标志位
    wheel_L_Flag = 0;       // 左转标志位
    ...
    Car_Spend = speed;      // 速度值
    Control(Car_Spend, Car_Spend);  // 电机驱动函数
    while(Stop_Flag != 0x01);       // 等待前进完成     //循迹完成的时候 Stop_Flag 为1
}


void Car_Time_Track(int8_t speed, uint16_t times)      //主车时间循迹函数    参数：速度，时间
{
    Stop_Flag = 0;          // 运行状态标志位
    Go_Flag = 0;            // 前进标志位
    wheel_L_Flag = 0;       // 左转标志位
...
void Car_RFID_Track(uint8_t speed,uint8_t card)            //循迹读卡函数   
{
    RFID_Track(speed);       //循迹
	while(Stop_Flag != 0x01)
	{
