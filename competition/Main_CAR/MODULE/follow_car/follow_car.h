...
typedef struct
{
	...
	void (*XSend_Licence_To_fromcar)(char*);
    void (*XSend_Licence_To_fromcar1)(char*);
	void (*XSend_Text_To_fromcar)(const char*);
}FollowCar_Typedef;

extern FollowCar_Typedef FollowCar_Data;
extern uint8_t fromcar_start[8];
extern uint8_t MainCar_start_Finish[8];
extern uint8_t Follow_Send_Licence_Data_Store[6];
extern uint8_t Follow_Send_Wireless_Open_Data_Store[3];
extern uint8_t Follow_Send_Weizhi;
extern uint8_t Follow_Send_Algorithm_Data_Store[3];
extern uint8_t Send_Init_Light_To_fromcar[8];  
extern uint8_t Send_RFID_Data_To_fromcar[8]; 
