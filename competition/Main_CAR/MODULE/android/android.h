...
extern uint8_t Two_Code_Init_Data_Store[3][200];   //存储识别二维码初始数据
...
extern uint8_t TFT_License_Data_Store[7];   //TFT车牌数据存储
extern uint8_t TFT_Traffic_Data_Store[6];   //TFT交通标志数据存储
extern uint8_t TFT_GraphCount_Data_Store[7]; //TFT图形数量数据存储 
extern uint8_t TFT_GraphColour_Data_Store[7]; //TFT图形颜色数据存储
extern uint8_t TFT_GraphAndColour_Data_Store[6]; //TFT图形(数量+颜色)数据存储
extern uint8_t TFT_Mask_Data_Store[6]; //TFT口罩行人数量
extern uint8_t TFT_Words_Data_Store[6]; //TFT文字识别存储数据
...
extern uint8_t TFT_Car_Type_Data_Store;
...
extern uint8_t Identify_TFT_Store[8];
