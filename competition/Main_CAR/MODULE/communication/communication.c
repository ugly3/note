...
//                        printf("shiji:%d ",Communication_Data.CarPort_Back_A_Infrared);
                        
                        uint8_t Buf[20]={0};/****************************************现场打开测试验证**********************************************/
                        sprintf((char*)Buf,"C:%d ",Communication_Data.CarPort_Back_A_Infrared);  /********现场打开******************************************************************************/
                        Send_InfoData_To_Fifo((char*)Buf,strlen((char*)Buf)); /***********现场打开**************************************************************************************/
                    }
                    else if(Zigb_Rx_Buf[1] == CarPort_Data.Device_B )          //判断为B设备
                    {
            ...
            if(Zigb_Rx_Buf[1] == 0x0C)   //**************************ETC系统标志物
            {
                ...
                {
                    Communication_Data.ETC_Open_Flag = 1;        //ETC闸门开启状态
                }
            }
         ...
         if(0x06 == Zigb_Rx_Buf[1])                   //**************************语音播报标志物
            ...
            if(0x01 == Zigb_Rx_Buf[1])    //********************************接收从车发过来的数据
            {
                ...
                {
                    if(0x01 == Zigb_Rx_Buf[3])
                    {
                        FollowCar_Data.FollowCar_Finish_Flag = 1;   // 主车启动
                        FollowCar_Data.xSend_Command_TO_FollowCar(MainCar_start_Finish);   //发送主车已启动
                    }
                    else if(0x02 == Zigb_Rx_Buf[3])
                        FollowCar_Data.FollowCar_Stat_Flag  = 1;   //从车完成启动
                }
                else if(0xA1 == Zigb_Rx_Buf[2] && 0xA2 == Zigb_Rx_Buf[3])    //接收从车发送的温度数据
                {
                    
                }
                else if(0xA2 == Zigb_Rx_Buf[2] && 0xA3 == Zigb_Rx_Buf[3])    //接收从车发送的距离数据
                {
                    
                }
                else if(0xA3 == Zigb_Rx_Buf[2] )    //从车传来的车牌前三位信息
                {
                    FollowCar_Data.Follow_Send_Licence_Data_Flag = 1;
                    Follow_Send_Licence_Data_Store[0] = Zigb_Rx_Buf[3];
                    Follow_Send_Licence_Data_Store[1] = Zigb_Rx_Buf[4];
                    Follow_Send_Licence_Data_Store[2] = Zigb_Rx_Buf[5];
                    Follow_Send_Licence_Data_Store[3] = Zigb_Rx_Buf[6];
                }
                else if(0xA4 == Zigb_Rx_Buf[2])    //从车传来的车牌后三位信息
                {
                    Follow_Send_Licence_Data_Store[4] = Zigb_Rx_Buf[3];
                    Follow_Send_Licence_Data_Store[5] = Zigb_Rx_Buf[4];
                }
                else if(0xA5 == Zigb_Rx_Buf[2])    //从车传来的文字数据
                {
                    
                }
                else if(0x01 == Zigb_Rx_Buf[2]) //从车传来的无线充电标志物开启码
                {
                    Follow_Send_Wireless_Open_Data_Store[0] = Zigb_Rx_Buf[3];
                    Follow_Send_Wireless_Open_Data_Store[1] = Zigb_Rx_Buf[4];
                    Follow_Send_Wireless_Open_Data_Store[2] = Zigb_Rx_Buf[5];
                }
                else if(0x02 == Zigb_Rx_Buf[2])//从车传来的位置数据
                {
                    Follow_Send_Weizhi = Zigb_Rx_Buf[3];
                }
                else if(0x03 == Zigb_Rx_Buf[2])//从车传来的算法参数数据
                {
                    Follow_Send_Algorithm_Data_Store[0] = Zigb_Rx_Buf[3];
                    Follow_Send_Algorithm_Data_Store[1] = Zigb_Rx_Buf[4];
                    Follow_Send_Algorithm_Data_Store[2] = Zigb_Rx_Buf[5];
                }
            }
...
//                    Send_InfoData_To_Fifo((char*)Buf1,strlen((char*)Buf1)); 
				...
				case 0x06:    //TFT图形数量识别成功
				{
                    ...
                    TFT_GraphCount_Data_Store[6] = 0x00;
//                    for(uint8_t i=0;i<6;i++)
                        printf("%s ",TFT_GraphCount_Data_Store);
					break;
				}
				case 0x07:    //TFT图形颜色识别成功 
				{
                    ...
                    TFT_GraphColour_Data_Store[6] = 0x00;
//                    for(uint8_t i=0;i<6;i++)
                        printf("%s ",TFT_GraphColour_Data_Store);
					break;
				}
				case 0x08: //TFT图形识别成功 （数量+颜色）
				{
					Android_Data.TFT_Graph_CountAndColour_Flag = 1;
                    if(Wifi_Rx_Buf[3]==0x01)    //数量
                    {
                    ...
                    else if(Wifi_Rx_Buf[3]==0x02)//颜色
                    {
                        TFT_GraphAndColour_Data_Store[3] = Wifi_Rx_Buf[4];
                        TFT_GraphAndColour_Data_Store[4] = Wifi_Rx_Buf[5];
                        TFT_GraphAndColour_Data_Store[5] = Wifi_Rx_Buf[6];
                    }
...
//					}
                ...
                case 0x11:  //车型识别
                {
                    ...
                    TFT_Car_Type_Data_Store = Wifi_Rx_Buf[3];
